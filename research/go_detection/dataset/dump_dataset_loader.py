...
def _read_raw_image_2(data_io: AssetIO, image_path: str, board_pts: torch.Tensor):
    """
    Returns tuple:
        - resized image
        - board_pts after the transformation
    ...
    upper_bound = torch.tensor([width, height])

    top_left = board_pts.min(dim=0).values
    bottom_right = board_pts.max(dim=0).values

    # Add padding to the points since the labels may not be accurate and also include some bg info
    padding = 50
    top_left = torch.max(torch.min(top_left - padding, upper_bound), lower_bound)
    bottom_right = torch.max(
        torch.min(bottom_right + padding, upper_bound), lower_bound
    )

    center_square = (top_left + bottom_right) / 2.0
    rectangle_half_length = (bottom_right - top_left) / 2.0
    ...
    # asset_io = AssetIO("/home/rmenon/Desktop/dev/projects/aigo")
    # asset_io.save_image("rishi_orig.png", orig_image)
    # asset_io.save_image("rishi_intermediate.png", intermediate_image)
    # asset_io.save_image("rishi_final.png", resized_image)
...
def load_dataset_path(data_point: RawDatasetPaths, data_io: AssetIO) -> RawDatasetPoint:
    ...
    image, board_pts = _read_raw_image_2(data_io, data_point.image_path, board_pts)
